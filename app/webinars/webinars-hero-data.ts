export const webinarsHeroContent = {
  badge: "Live Sessions",
  heading: "Learn from the people who built the framework.",
  description:
    "DPD webinars bring Persona-Based Cognitive Alignment to life. Join coaches, leaders, and team builders for live sessions that go beyond theory — with real examples, open Q&A, and a community that gets it.",
  cta: { label: "Reserve Your Spot", href: "/contact" },
  secondaryCta: { label: "Browse past sessions", href: "#recordings" },
  sessionDetails: [
    "60-minute live sessions",
    "Open Q&A with DPD experts",
    "Access to session recordings",
    "Free for all attendees",
  ],
} as const;
